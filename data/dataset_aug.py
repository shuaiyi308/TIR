...
class SetDataset:
  def __init__(self, data_file, batch_size, transform):
    ...
    self.cl_list = np.unique(self.meta['image_labels']).tolist()  ##0 1 2 3 4...

    self.sub_meta = {}
    for cl in self.cl_list:  # 0 1 2 ... 9
      self.sub_meta[cl] = []

    for x,y in zip(self.meta['image_names'],self.meta['image_labels']):  ## 为每个类别i 创建了列表，包含所有的image_name
...
##################################
class SubDataset:
  def __init__(self, sub_meta, cl, transform, target_transform=identity, min_size=50):
