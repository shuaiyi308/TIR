...
class PlainMultiheadAttentionLoRA(nn.Module):
    def __init__(
            ...
            r: int = 0, 
            lora_alpha: int = 1, 
            dropout_rate:float = 0.,offset_groups=4,patch_dropout=0.,
            **kwargs
        ):
        super().__init__()
    ...
    def scaled_dot_product_attention(self, query, key, value, attn_mask=None, dropout_p=0.0, 
            is_causal=False, scale=None, enable_gqa=False, params=None) -> torch.Tensor:
        ...
        #attn_weight = attn_weight * 1.5
  

        if attn_weight.shape[-1] == 77:

            
            attn_weight = torch.softmax(attn_weight, dim=-1) # [b, h, t, t]
            dropout_p =0#
    ...
    def forward_module(
        ...
        #print('beforeatt',q.shape)
       
       

        attn_output = self.scaled_dot_product_attention(q, k, v, attn_mask, dropout_p, is_causal, params=params)
